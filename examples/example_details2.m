...
% Folder where the epochs will be saved
Path2DataEPCH = fullfile(Path0,'DATA','epch');
...
for sbj=1:length(files2pp)
    ...
    if ~exist(Path2DataPrp, 'dir'), mkdir(Path2DataPrp), end
...
for sbj=1:length(files2pp)
    ...
    % ---------------------------------------------------------------------
    % Save the ERPs
    if ~exist(Path2DataEPCH, 'dir'), mkdir(Path2DataEPCH), end
    [~,name,~] = fileparts(EEG.filename);
    newfilename = ['epch_' name];
    pop_saveset(EEG, newfilename,Path2DataEPCH);
end

%% ========================================================================
%% ERPs PER SUBJECT - average per condition

files2epoch = dir(fullfile(Path2DataEPCH,'epch_*.set'));
files2epoch = files2epoch(~ismember({files2epoch.name},{'.', '..'}));
for sbj=1:length(files2epoch)
    
    FilesnameIn = files2epoch(sbj).name;
    
    % ---------------------------------------------------------------------
    % Load the data
    EEG = pop_loadset(fullfile(Path2DataEPCH, FilesnameIn) );
    ...
    % ---------------------------------------------------------------------
    % Save the ERPs
    if ~exist(Path2DataERP, 'dir'), mkdir(Path2DataERP), end
    [~,name,~] = fileparts(EEG.filename);
...
% -------------------------------------------------------------------------
% Print a preprocessing report for all the subjetcs
eega_printsummary('erp_ephc_prp*.set', Path2DataERP, Perp.report.patternname, Perp.report.patternn, 0);


%% ========================================================================
%% SELECT EPOCHS - using the F field added by APICE

files2erp = dir(fullfile(Path2DataEPCH,'epch_*.set'));
files2erp = files2erp(~ismember({files2erp.name},{'.', '..'}));
for sbj=1:length(files2erp)
    
    FilesnameIn = files2erp(sbj).name;
    ...
    % ---------------------------------------------------------------------
    % Select epochs in a given condition
    EEGcnd = eega_separatebyfactors(EEG, Perp.avgcnd);
       
    % ---------------------------------------------------------------------
    % Save the data in each condition in a separate file
    cnd = fieldnames(EEGcnd);
    for i=1:length(cnd)
        EEG = EEGcnd.(cnd{i});
        [~,name,~] = fileparts(EEG.filename);
        newfilename = [cnd{i} '_' name];
        pop_saveset(EEG, newfilename,Path2DataEPCH);
    end
