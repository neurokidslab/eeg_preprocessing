function [todss,pwr0,pwr1]=nt_dss0(c0,c1,keep1,keep2,minkeep)
...
if nargin<5||isempty(minkeep); minkeep=1; end
...
[topcs1,evs1]=nt_pcarot(c0,keep1,keep2,minkeep);
evs1=abs(evs1);

% truncate PCA series if needed
if ~isempty(keep1) && size(topcs1,2)>keep1 && minkeep<keep1    % I added here this --> && size(topcs1,2)>keep1  
    topcs1=topcs1(:,1:keep1); evs1=evs1(1:keep1); 
end
if ~isempty(keep2)
    idx=find(evs1/max(evs1)>keep2);
    if minkeep<length(idx)
        topcs1=topcs1(:,idx); evs1=evs1(idx);
    else
        topcs1=topcs1(:,1:minkeep); evs1=evs1(1:minkeep);
    end
end
...
[topcs2,evs2]=nt_pcarot(c2,size(topcs1,2),[],minkeep); % I change this line to avoid having different number of components
