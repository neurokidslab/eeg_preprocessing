...
function TMPREJ = eega_plot_rejection(EEG, plotbct, plotbt, plotbc, winlength, varargin)
...
if ~OK
    error('eega_plot_rejection: Non recognized inputs')
end
...
command = [...
    'EEG = eega_eegplot2artifacts(EEG,TMPREJ);'];

TMPREJ = eegplot(EEG.data,'srate',EEG.srate,'winlength',winlength,'color',chcol,...
    'winrej',winrej,'command',command);
