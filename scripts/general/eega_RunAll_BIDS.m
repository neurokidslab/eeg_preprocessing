...
% for all subjects in the input folder. The data is organized in one folder
% per subject (BIDS).
%
% INPUTS
% filenames     string to selct the input files
% path2data     path where the subject folders are
%
% OPTIONAL INPUTS
...
function eega_RunAll_BIDS( filenames, path2data, varargin )
...
%% ========================================================================
%% List of files
SBJs = eega_getfilesinfolders(path2data, filenames);

if isempty(SBJs)
    warning('eega_RunAll_BIDS: No files were found')
    return
end
...
for s = 1:length(SBJs)  % loop over subjects
    ...
    [pathSbj,nameSubj,extSbj] = fileparts(SBJs{s});
    ...
    if runto==3 || runto==2
        ...
        fullnameOut = fullfile(pathSbj, nameOut);
    ...
    if do
        ...
        if strcmp(thefunctions{1},'eega_importdata') ||...
            ...
            EEG = thefun( SBJs{s}, inputs{:} );
            fun_i = 3;
            dosave = 1;
            
        else
            if strcmp(extSbj,'.set')
                EEG = pop_loadset( [nameSubj,extSbj], pathSbj );
                fun_i = 1;
            elseif strcmp(extSbj,'.mat')
                EEG = load( SBJs{s} );
                EEG = EEG.EEG;
                fun_i = 1;
            end
        ...
        if dosave 
            if strcmp(saveformat,'set')
                pop_saveset( EEG, nameOut, pathSbj);
            elseif strcmp(saveformat,'mat')
                save(fullfile(pathSbj,nameOut),'EEG')
            end
        end
