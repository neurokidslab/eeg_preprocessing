...
% -------------------------------------------------------------------------
% Ana Flo July  2020
% By setting usealphaband=0 the alpha band feature is not used for 
% classification
% -------------------------------------------------------------------------

function [artcomps, info] = MARA_babies(EEG,usealphaband)
try 
    %%%%%%%%%%%%%%%%%%%%
    ...
    idxfeats = true(1,size(fv_tr.x,1));
    if ~usealphaband
        idxfeats(5) = 0; % do not use alpha power
    end
    [C, foo, posterior] = classify(feats(idxfeats,:)',fv_tr.x(idxfeats,:)',fv_tr.labels(1,:));
...
for ic=1:length(icacomps(:,1))  %for each component
    ...
    % the error was calculated in the band [8 15], now it is calculated in
    % the band [2 40]. By Ana Flo July 2nd 2020
    ts_8to15 = freq(find(freq == 2) : find(freq == 40));
    fs_8to15 = pxx(find(freq == 2) : find(freq == 40)); 
...
%        
%     p = 0;
%     for i = alphaband(1):alphaband(end)
%         p = p + pxx(find(freq == i,1));
%     end
%     Hz8_13 = p / (diff(alphaband)+1);
    
%     fidx_fit = (freq>=(alphaband(1)) & freq<=13);
%     fidx_bnew = (freq>=alphaband(1) & freq<=alphaband(2));
%     fidx_borg = (freq>=8 & freq<=13);
% 
%     ybnew = pxx(fidx_bnew);
%     yborg = pxx(fidx_borg); 
%     Ybnew = myfun(fittedmodel, freq(fidx_bnew));    
%     Yborg = myfun(fittedmodel, freq(fidx_borg));    
%     Hz8_13 = sum(yborg) / (diff([8 13])+1) + sum( ybnew - Ybnew ) / (diff(alphaband)+1);
%     
    
%     fidx_fit = (freq>=(alphaband(1)) & freq<=13);
%     fidx_bnew = (freq>=alphaband(1) & freq<=alphaband(2));
%     fidx_borg = (freq>=8 & freq<=13);
%     x = log10(freq(fidx_fit));
%     y = pxx(fidx_fit);
%     p = polyfit(x,y,1);
%     ybnew = pxx(fidx_bnew);
%     yborg = pxx(fidx_borg);
%     Ybnew = polyval(p,log10(freq(fidx_bnew)));    
%     Hz8_13 = sum(yborg) / (diff([8 13])+1) + sum( ybnew - Ybnew ) / (diff(alphaband)+1);
